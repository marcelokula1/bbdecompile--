...
public class CraftersScript : MonoBehaviour
{
	...
	private void Start()
	{
		...
		this.normalSprite = this.spriteImage.sprite;
		this.baseSpeed = this.agent.speed;
	}
	...
	private void Update()
	{
		...
		if (this.angerCooldown > 0f)
		{
			this.angerCooldown -= Time.deltaTime;
		}
		...
		{
			if (this.agent.speed < 45f)
			{
				this.agent.speed = this.agent.speed + 10f * Time.deltaTime; // Slowly increase the speed
			}
	...
	{
		if (!(this.gc.notebooks >= this.gc.maxNotebooks & this.angerCooldown <= 0f)) // If the player has more then 7 notebooks
		{
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		{
			GameObject attacker = GameObject.Instantiate(this.attackingCrafters);
			attacker.transform.position = base.transform.position + Vector3.up * 3f;
			attacker.GetComponent<CraftersAttackerScript>().playerTransform = this.player;
			attacker.GetComponent<CraftersAttackerScript>().crafters = base.gameObject;
	...
	public void GiveConsequence()
    {
        ...
        {
		this.angerCooldown = 3f;
		this.anger = 0f;
		this.gettingAngry = false;
		this.angry = false;
		this.agent.speed = this.baseSpeed;
		this.spriteImage.sprite = this.normalSprite;
	...
	// Token: 0x040006A1 RID: 1697
	public GameObject attackingCrafters;
